...
alter table public.books add constraint books_owner_id_fkey foreign key (owner_id) references auth.users(id) on delete cascade;

alter table public.clients drop constraint if exists clients_owner_id_fkey;
alter table public.clients add constraint clients_owner_id_fkey foreign key (owner_id) references auth.users(id) on delete cascade;
